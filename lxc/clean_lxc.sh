...
for container in $(pct list | awk '{if(NR>1) print $1}'); do containername=$(pct exec "$container" hostname) &&\
    ...
    pct exec $container -- bash <(curl -fsSL https://github.com/tteck/Proxmox/raw/main/misc/clean.sh) &&\
    pct exec $container -- bash -c "rm -rf /var/lib/apt/lists/* && apt-get updaten" &&\
    echo ---; done
